...
def main():
    ...
    for x in range (0, 17):
        for y in range(0, 20):
            #check to the left
            if (x > 2):
                current = matrix[x][y] * matrix[x-1][y] * matrix[x-2][y] * matrix[x-3][y]
                if (current > maximum):
                    maximum = current
            #check to the right
            if (x < 17):
            ...
            #check below
            if (y > 2):
                currrent = matrix[x][y] * matrix[x][y-1] * matrix[x][y-2] * matrix[x][y-3]
                if(current > maximum):
                    maximum = current
            #check above
            if (y < 17):
                current = matrix[x][y] * matrix[x][y+1] * matrix[x][y+2] * matrix[x][y+3]
                if (current > maximum):
                    maximum = current
            #check left diagonal
            if (x > 2 and y < 17):
                current = matrix[x][y] * matrix[x-1][y+1] * matrix[x-2][y+2] * matrix[x-3][y+3]
                if (current > maximum):
                    maximum = current
